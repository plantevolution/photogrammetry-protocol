...
```{r flexible-tripod, echo=FALSE, message=FALSE, fig.cap="Flexible tripod.", out.width = '33%', fig.align='center'}
knitr::include_graphics("Figures/tripod.png")
```

```{r collapsible-tripod, echo=FALSE, message=FALSE, fig.cap="Collapsible tripod.", out.width = '33%', fig.align='center'}
knitr::include_graphics("Figures/Collapsible_tripod.png")
```
...
```{r camera-arrows-no-FS, echo=FALSE, message=FALSE, fig.cap="Camera (Canon T2i/550D) and lens (60mm f/2.8 Macro lens) used to take RAW photos. The red arrows (from top to bottom) depict the button to get manual focus, the ISO button and the Manual Exposure mode dial.", out.width = '33%', fig.align='center'}
knitr::include_graphics("Figures/camera_arrows.png")
```

```{r camera-settings-no-FS, echo=FALSE, message=FALSE, fig.cap="Camera settings interface of the Canon t2i/550D (no focus stacking).", out.width = '50%', fig.align='center'}
...
If you decide to do focus stacking for your project, check the manual of your camera for optimal parameters during focus bracketing as they may slightly differ from what we use. For a Canon EOS 90D camera, we use the following settings: M (Manual Exposure) mode dial, 1/20 exposure time, F/11 aperture, ISO 100, Auto Focus (AF) lens focus mode, RAW image format (Figure \@ref(fig:camera-settings-FS), Figure \@ref(fig:camera-arrows-FS)).We usually use the *Zone AF* method and enable *Continuous AF* (Shooting Menu, Tab 6). 

Turn off the *Image Review* option (Shooting Menu, Tab 1) to prevent delays after taking each set of pictures. To enable focus bracketing on EOS 90D, press the "START-STOP" button to enter the *Live View Shooting Mode*, then enable *Focus Bracketing* (Shooting Menu, Tab 5). If using an EF 100mm f/2.8L MACRO IS USM lens, disable *Exposure Smoothing* (option within *Focus Bracketing*), otherwise it may cause changes in image brightness. 

The optimal number of shots and focus increments depend on the size of the flower, as well as its position and distance from the camera. For example, from the highest camera position, 8 images with a focus increment of 4 works good for a ~2.5x1x1cm (LxWxH) tubular flower, but from an en-face camera angle (i.e. the flower opening facing the camera), 10-12 images with a focus increment of 3 may work better. Test what parameters would produce the smallest number of shots with each part of your flower in focus in each case. Generally, the higher the number of shots, the smaller the focus increments can be.
...
```{r camera-arrows-FS, echo=FALSE, message=FALSE, fig.cap="Camera (Canon EOS 90D) and lens (EF 100m f/2.8L MACRO IS USM) used to take RAW photos for focus stacking. Arrow on the lens: Focus mode button set to Auto Focus (AF). Arrows on the camera body (from top to bottom): ISO button, Manual Exposure mode dial and \"START-STOP\" button (for *Live View Shooting Mode*).", out.width = '75%', fig.align='center'}
knitr::include_graphics("Figures/Canon_EOS_90D_buttons.png")
```

```{r camera-settings-FS, echo=FALSE, message=FALSE, fig.cap="Camera settings interface of the Canon EOS 90D, used for projects with focus stacking.", out.width = '50%', fig.align='center'}
knitr::include_graphics("Figures/Canon_EOS_90D_settings.png")
```

```{r focus-stacking-settings, echo=FALSE, message=FALSE, fig.cap="Focus bracketing settings of the Canon EOS 90D.", out.width = '50%', fig.align='center'}
knitr::include_graphics("Figures/Focus_bracketing_settings.png")
```
...
knitr::include_graphics("Figures/Custom_WB_setting.png")
```

```{r WB2, echo=FALSE, message=FALSE, fig.cap="How to select a custom white balance.", out.width = '50%', fig.align='center', fig.show = 'hold'}
knitr::include_graphics("Figures/Custom_WB_selection.png")
```


## Turntable settings and preparation
...
6.  Place the white background paper disk with the foam block on the turntable to contrast with
