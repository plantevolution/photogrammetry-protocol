...
```{r agisoft, echo=FALSE, message=FALSE, fig.cap="Agisoft Metashape", out.width = '33%', fig.align='center'}
...
with *BuildModel/tvl1_mesh* and select the value as *False* (figure
[\[metashape_tweaks\]](#metashape_tweaks){reference-type="ref"
reference="metashape_tweaks"}). Additionally, to use the anterior
version of the depth maps generation process, add ONCE the tweak :
*BuildDepthMaps/pm_enable* and set the value to *False* (figure
[\[metashape_tweaks\]](#metashape_tweaks){reference-type="ref"
reference="metashape_tweaks"}).

![](Figures/Metashape_tweaks_1.png){width="100%"}

![](Figures/Metashape_tweaks_2.png){width="100%"}
...
3.  Click on *Masks*, *Import Masks* and in the box that appears select
    method \"From Background\", operation *Replacement*.
...
![Right click on an image to select a mask to
import.](Figures/Metashape_mask_right_click.png){width="100%"}

![Import mask from a black image *background.jpg*, and select a
...
images).](Figures/Metashape_masks_tolerance.png){width="100%"}
...
4.  Then, you will need to create ONCE a Photoshop action, that will be
    ...
    ![Record a new action called *Automatic Mask - 3D
    reconstruction*](Figures/mask_1.png){width="1\\linewidth"}

    ![When you reopen your photo, don't forget to remove this extra task
    in your action.](Figures/mask_2.png){width="1\\linewidth"}

    ![The action should include *Select Subject, Fill, Inverse, Fill,
    Save, Close*, and you can batch process this action to a specific
    folder of copied photos to create
    masks.](Figures/mask_3.png){width="1\\linewidth"}

    ![Apply the action to the folder of copied photos called
    *Chunk1-masks*.](Figures/mask_4.png){width="1\\linewidth"}
...
buttons (Figure [25](#tools_masks){reference-type="ref"
reference="tools_masks"}). You can then use the selection tools to
select areas that are not the flower and add the selection to the mask
(Figure [25](#tools_masks){reference-type="ref"
reference="tools_masks"}). You also remove the scale and the
...
![Selection tools and add selection to mask
tool](Figures/tools_masks.png){#tools_masks width="50%"}
...
4.  If you are aligning several chunks of photos, it is possible to run
    ...
    *all chunks* or select specific ones \> add parameters listed above
    (Figure [26](#batch_alignment){reference-type="ref"
    reference="batch_alignment"}).

    ![Align photos in multiple
    chunks](Figures/metashape_batch_align.png){#batch_alignment
    width="50%"}
...
9.  **Optional** If the camera alignment is not satisfactory, it is
    ...
    [\[removing_points\]](#removing_points){reference-type="ref"
    reference="removing_points"}):
...
![Use the selection tool to remove background
points](Figures/metashape_delete_selectedpoints.png){#model_orientation1
width="100%"}

![Use the gradual selection tool to remove additional mis-calculated
points](Figures/metashape_gradual_selection.png){#model_orientation2
width="100%"}
...
2.  Go to *Workflow* \> *Align Chunks*, select the chunks you want to
    align together, set the method as *Point based* (Figure
    [29](#Align_chunk){reference-type="ref" reference="Align_chunk"}).
...
Figure [30](#show_aligned_chunk){reference-type="ref"
reference="show_aligned_chunk"}). The different chunks should be well
...
![Align chunks](Figures/metashape_align_chunks.png){#Align_chunk
width="100%"}

![Show aligned chunks to verify their
positions](Figures/metashape_show_aligned_chunks.png){#show_aligned_chunk
width="90%"}
...
3.  In the dialog box, make sure that *Source Data* is on *Depth maps*,
    *Quality* and *Face Count* on *High* (Figure
    [31](#fig:3d_mesh){reference-type="ref" reference="fig:3d_mesh"}).
...
![Build 3D mesh](Figures/metashape_build_mesh.png){#fig:3d_mesh
width="50%"}
...
2.  In the left panel, select both markers and click on the icon *Add
    scale* (Figure [32](#metashape_scale){reference-type="ref"
    reference="metashape_scale"}).
...
![Add a scale using
landmarks](Figures/metashape_add_scale_2.png){#metashape_scale
width="90%"}
...
![](Figures/metashape_orientation.png){width="100%"}

![](Figures/metashape_orientation_2.png){width="100%"}
